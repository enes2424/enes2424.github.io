...
const branchDescriptions = {
  tr: {
    ...
    "42-Kocaeli-Mobile-0":
      "Bu proje, 42 Okulu Mobile Piscine eğitiminin başlangıç modülü olan Flutter ve UI geliştirme çalışmasıdır. Dart programlama dili ve Flutter widget sisteminin (Stateless/Stateful) temellerini kullanarak; dinamik arayüz yönetimi, responsive (duyarlı) tasarım prensipleri ve karmaşık layout mimarilerini kapsar. Proje içeriğinde yer alan tam fonksiyonel hesap makinesi uygulaması, işlem önceliği tabanlı özel bir parsing algoritması ve kapsamlı hata yönetimi mekanizmalarıyla zenginleştirilmiştir. Bu çalışma, modern mobil uygulama geliştirme süreçlerine ve çok platformlu UI tasarım standartlarına giriş niteliğinde profesyonel bir örnektir.",
    "42-Kocaeli-Mobile-1":
      "Bu proje, 42 Okulu Mobile Piscine eğitiminin ikinci modülü olan ileri seviye kullanıcı arayüzü (UI) ve navigasyon çalışmasıdır. Flutter'ın 'TabBar' ve 'TabController' yapılarını kullanarak çok sayfalı bir hava durumu uygulaması geliştirilmesini, dinamik arama sistemlerini ve 'MediaQuery' ile 'LayoutBuilder' tabanlı ileri seviye responsive (duyarlı) tasarım tekniklerini kapsar. Proje; widget yaşam döngüsü (lifecycle) yönetimi, 'SingleTickerProviderStateMixin' entegrasyonu ve ekran boyutlarına göre uyum sağlayan adaptive (uyarlanabilir) tasarım stratejilerini uygulamalı olarak sergiler. Bu çalışma, karmaşık mobil navigasyon akışları ve profesyonel varlık (asset) yönetimi konularında derinleşmek için tasarlanmış kapsamlı bir örnektir.",
    "42-Kocaeli-Mobile-2":
      "Bu proje, 42 Okulu Mobile Piscine eğitiminin üçüncü modülü olan API entegrasyonu ve ileri seviye asenkron veri yönetimi çalışmasıdır. 'http' kütüphanesi ile dış kaynaklı hava durumu API'larından (Open-Meteo) veri çekilmesini, 'geolocator' ile gerçek zamanlı GPS konum servislerinin yönetimini ve 'FutureBuilder' tabanlı modüler bir asenkron mimariyi kapsar. Uygulama; anlık arama önerileri (autocomplete), gelişmiş hata yönetimi mekanizmaları ve çalışma zamanı izin kontrolü (runtime permission) gibi üretim seviyesinde mobil uygulama standartlarını başarıyla sergiler. Bu çalışma, gerçek dünya verileriyle etkileşime giren, verimli ve ölçeklenebilir mobil çözümler üretmek için tasarlanmış kapsamlı bir örnek teşkil eder.",
    "42-Kocaeli-Mobile-3":
      "Bu proje, 42 Okulu Mobile Piscine eğitiminin dördüncü modülü olan veri görselleştirme (data visualization) ve ileri seviye kullanıcı arayüzü tasarımı çalışmasıdır. 'FL Chart' kütüphanesini kullanarak interaktif grafikler, sıcaklık ve rüzgar hızı trendlerini gösteren çizgi grafikler (line charts) ve özelleştirilmiş arka plan varlıklarını içeren profesyonel bir hava durumu uygulaması sunar. Proje; karmaşık veri setlerinin görselleştirilmesi, grafik performans optimizasyonu ve dokunmatik etkileşimli grafik yönetimi gibi ileri düzey konuları başarıyla uygular. Bu çalışma, ham verilerin kullanıcı dostu ve görsel olarak etkileyici modellere dönüştürülmesini sağlayan yüksek kaliteli bir mobil uygulama örneğidir.",
    "42-Kocaeli-Mobile-4":
      "Bu proje, 42 Okulu Mobile Piscine eğitiminin beşinci modülü olan modern durum yönetimi (state management) ve backend entegrasyonu çalışmasıdır. 'Riverpod' kütüphanesi ile reaktif durum yönetimi, 'Supabase' ile bulut tabanlı PostgreSQL veritabanı işlemleri (CRUD) ve 'Go Router' ile deklaratif navigasyon yapılarını kapsar. Uygulama; Google ve GitHub üzerinden OAuth 2.0 sosyal giriş mekanizmaları, kullanıcıya özel veri filtreleme ve ruh hali takibi (mood tracking) gibi ileri seviye full-stack mobil uygulama yetkinliklerini başarıyla sergiler. Bu çalışma, kimlik doğrulama korumalı (route guards) ve ölçeklenebilir bir mimariye sahip, profesyonel standartlarda bir mobil çözüm örneğidir.",
    "42-Kocaeli-Mobile-5":
      "Bu proje, 42 Okulu Mobile Piscine eğitiminin altıncı modülü olan ileri seviye kullanıcı arayüzü (UI) bileşenleri ve takvim entegrasyonu çalışmasıdır. 'Table Calendar' kütüphanesi ile etkileşimli takvim yönetimi, 'Go Router' üzerinden 'Shell Route' mimarisi ile kalıcı alt navigasyon çubuğu ve tarihe göre filtrelenebilir içerik yönetimi özelliklerini kapsar. Uygulama; aylık, iki haftalık ve haftalık görünüm modları, çok sayfalı mimari ve karmaşık durum koordinasyonu gibi ileri düzey mobil uygulama desenlerini başarıyla uygular. Bu çalışma, bileşen tabanlı tasarım ve takvim odaklı veri organizasyonu konularında profesyonel standartlarda bir mobil çözüm örneği sunar.",

    // 42-Kocaeli-Ocaml-Piscine
    "42-Kocaeli-OCAML-1-BasicSyntaxAndSemantics-0":
      "Bu proje, 42 Okulu OCaml Piscine eğitiminin başlangıç modülü olan fonksiyonel programlama ve temel dil sözdizimi çalışmasıdır. OCaml dilinin güçlü tip sistemini ve immutability (değişmezlik) prensiplerini kullanarak; özyinelemeli (recursive) fonksiyonlar, yüksek seviyeli fonksiyonlar (higher-order functions) ve pattern matching tekniklerini kapsar. Proje içeriğinde; hızlı üs alma algoritmaları, ROT-N şifreleme ve karmaşık string manipülasyonu işlemleri fonksiyonel yaklaşım disipliniyle başarıyla uygulanmıştır. Bu çalışma, fonksiyonel paradigmaya geçiş ve algoritmik problemlerin özyinelemeli yapılarla çözümü konusunda temel teşkil eden profesyonel bir örnektir.",
    "42-Kocaeli-OCAML-2-RecursionAndNigherOrderFunctions-0":
      "Bu proje, 42 Okulu OCaml Piscine eğitiminin ikinci modülü olan ileri seviye özyineleme (recursion) ve yüksek dereceli fonksiyonlar (higher-order functions) çalışmasıdır. 'Tail recursion' optimizasyonu ile bellek verimliliği, karşılıklı özyineleme (mutual recursion) ve opsiyonel parametreli esnek fonksiyon yapılarının kullanımını kapsar. Proje içeriğinde; Ackermann, Fibonacci ve Takeuchi gibi karmaşık matematiksel algoritmaların yanı sıra Leibniz serisi ile sayısal yaklaşım yöntemleri fonksiyonel programlama disipliniyle başarıyla uygulanmıştır. Bu çalışma, polimorfik fonksiyon yapıları ve algoritmik performans optimizasyonu konularında derinlemesine teknik yetkinlik sergileyen profesyonel bir örnektir.",
    "42-Kocaeli-OCAML-3-PatternMatchingAndDataTypes-0":
      "Bu proje, 42 Okulu OCaml Piscine eğitiminin üçüncü modülü olan desen eşleştirme (pattern matching) ve özel veri türleri (data types) çalışmasıdır. OCaml'in 'variant' ve 'record' türlerini kullanarak moleküler biyolojinin temel dogmasını (DNA → RNA → Protein) modelleyen algoritmik bir simülasyon sunar. Proje içeriğinde; 'Run-length encoding', Gray kodu üretimi ve 'Look-and-say' dizisi gibi klasik algoritmaların yanı sıra genetik kod çevrimi gibi biyoinformatik temelli sistemler fonksiyonel yaklaşımla geliştirilmiştir. Bu çalışma, karmaşık veri yapılarının güvenli tip sistemi ile modellenmesi ve kapsamlı örüntü eşleştirme tekniklerinin yazılım mimarisindeki gücünü sergileyen profesyonel bir örnektir.",
    "42-Kocaeli-OCAML-4-OcamlsModulsLanguage-1":
      "Bu proje, 42 Okulu OCaml Piscine eğitiminin dördüncü modülü olan modül sistemi ve gelişmiş veri yapıları çalışmasıdır. Iskambil kartı sistemi mimarisini temel alarak; '.mli' arayüz dosyaları ile kapsülleme (encapsulation), modül birleşimi (composition) ve özel veri yapılarının hiyerarşik organizasyonunu kapsar. Proje içeriğinde; kart renkleri, değerleri ve kart nesnelerinden oluşan 52'lik tam bir destenin Fisher-Yates benzeri karıştırma algoritmalarıyla yönetimi tip güvenli bir şekilde geliştirilmiştir. Bu çalışma, büyük ölçekli fonksiyonel projelerde modüler mimari tasarımı, API güvenliği ve gerçek dünya domain modellemesi konularında profesyonel yetkinlik sergiler.",
    "42-Kocaeli-OCAML-5-ImperativeFeatures-1":
      "Bu proje, 42 Okulu OCaml Piscine eğitiminin beşinci modülü olan imperative (zorunlu) programlama özellikleri ve sistem etkileşimi çalışmasıdır. OCaml'in fonksiyonel yapısının yanındaki 'mutable' veri yapılarını, 'Unix' modülü üzerinden sistem çağrılarını, dosya giriş/çıkış (I/O) işlemlerini ve kapsamlı 'exception' yönetimini kapsar. Proje içeriğinde; özel referans tipi implementasyonu, N-boyutlu Euclidean mesafe hesaplama algoritmaları ve makine öğrenmesi veri setleri için CSV ayrıştırma (parsing) sistemleri geliştirilmiştir. Bu çalışma, performans odaklı mutable koleksiyon yönetimi, sistem programlama pratikleri ve gerçek dünya verilerinin işlenmesi konularında profesyonel teknik yetkinlik sergiler.",
    "42-Kocaeli-OCAML-6-Functors-1":
      "Bu proje, 42 Okulu OCaml Piscine eğitiminin altıncı modülü olan fonktörler (functors) ve gelişmiş modül sistemleri çalışmasıdır. OCaml'in modüller arası fonksiyonel yapısını kullanarak; parametreleştirilmiş modül tasarımı, üst düzey modül imzaları ve generic programlama tekniklerini kapsar. Proje içeriğinde; 'Fixed-point' aritmetik sistemleri, farklı veri tipleriyle matematiksel ifadeleri değerlendiren generic 'expression evaluator' ve standart kütüphane fonktörlerinin özelleştirilmiş implementasyonları geliştirilmiştir. Bu çalışma, büyük ölçekli yazılım mimarilerinde kodun yeniden kullanılabilirliği, tip soyutlama ve sıfır maliyetli soyutlama prensipleri konularında profesyonel teknik yetkinlik sergiler.",
    "42-Kocaeli-OCAML-7-ObjectOrientedProgramming-1":
      "Bu proje, 42 Okulu OCaml Piscine eğitiminin yedinci modülü olan nesne yönelimli programlama (OOP) çalışmasıdır. OCaml'in fonksiyonel temelleri üzerinde yükselen; sınıf tanımlama, kalıtım (inheritance), sarmalama (encapsulation) ve özele indirgenmiş metod yapılarını 'Doctor Who' evreni üzerinden modelleyerek kapsar. Proje içeriğinde; 'Doctor', 'Dalek' ve 'People' sınıflarıyla nesneler arası karmaşık etkileşimler, durum yönetimi (mutable state) ve 'composition' prensibiyle kurgulanmış bir savaş simülasyonu sistemi geliştirilmiştir. Bu çalışma, fonksiyonel dillerde nesne tabanlı modelleme yeteneklerini sergileyen ve tip güvenli OOP mimarileri inşa eden profesyonel bir örnektir.",
    "42-Kocaeli-OCAML-8-ObjectOrientedProgramming-2":
      "Bu proje, 42 Okulu OCaml Piscine eğitiminin sekizinci modülü olan sanal sınıflar (virtual classes) ve gelişmiş kimyasal modelleme çalışmasıdır. OCaml'ın nesne yönelimli yeteneklerini kullanarak; abstract base class yapılarını, nesne bileşimini (composition) ve karmaşık sınıfsal hiyerarşileri kimya temasıyla modeller. Proje içeriğinde; atom ve molekül nesnelerinden başlayarak, alkan serilerinin (CnH2n+2) matematiksel formülasyonu ve alkan yanma reaksiyonlarının stoikiometrik olarak otomatik dengelenmesini sağlayan gelişmiş bir algoritma sistemi geliştirilmiştir. Bu çalışma, bilimsel verilerin nesne tabanlı mimariyle modellenmesi, hata yönetimi ve karmaşık matematiksel mantığın (stoichiometry) yazılım ortamına aktarılması konularında üst düzey teknik yetkinlik sergiler.",
    "42-Kocaeli-OCAML-9-MonoidsAndMonads-3":
      "Bu proje, 42 Okulu OCaml Piscine eğitiminin dokuzuncu modülü olan ileri düzey fonksiyonel programlama soyutlamaları, monoidler ve monadlar çalışmasıdır. Kategori teorisi prensiplerini temel alarak; saat aritmetiği gibi matematiksel monoidleri, generic hesaplama yetenekleri sağlayan fonktörleri ve hata yönetimi ile deterministik olmayan hesaplamalar için özelleşmiş 'Try' ve 'Set' monadlarını kapsar. Proje içeriğinde; cebirsel yasaların kod hiyerarşisine uygulanması, fonksiyonel kompozisyon teknikleri ve tip güvenli hesaplama bağlamları geliştirilmiştir. Bu çalışma, karmaşık yazılım problemlerini saf fonksiyonel paradigmalar ve matematiksel kesinlikle çözme konularında üst düzey teknik uzmanlık sergiler.",

    // 42-Kocaeli-Django-Piscine
    "42-Kocaeli-Django-0-Initiation":
      "Bu proje, 42 Okulu Django Piscine eğitiminin ilk modülü olan temel web teknolojileri (HTML, CSS, JavaScript) çalışmasıdır. Web geliştirmenin temellerini; semantik HTML5 yapısı, client-side form validasyonu, CSS animasyonları ve jQuery ile medya (audio API) yönetimi üzerinden kapsamlı bir şekilde ele alır. Proje içeriğinde; HTTP yönlendirmelerini takip eden Bash betikleri, responsive blog tasarımları ve JavaScript fonksiyon zinciri yönetimleri gibi hem frontend hem de temel sistem etkileşimleri geliştirilmiştir. Bu çalışma, modern web standartları, DOM manipülasyonu ve varlık yönetimi konularında profesyonel bir başlangıç yetkinliği sergiler.",
    "42-Kocaeli-Django-1-Starting":
      "Bu proje, 42 Okulu Django Piscine eğitiminin ikinci modülü olan Python temelleri ve veri yapıları çalışmasıdır. Python'un temel veri tiplerinden başlayarak; gelişmiş sözlük (dictionary) manipülasyonları, hata yönetimi (exception handling), dosya tabanlı veri işleme ve komut satırı argüman yönetimi konularını kapsar. Proje içeriğinde; büyük veri setlerinin sıralanması, karmaşık veri yapıları üzerinde tersine arama (reverse lookup) teknikleri ve periyodik tablo verilerinden programatik olarak semantik HTML tabloları oluşturma gibi veri işleme odaklı uygulamalar geliştirilmiştir. Bu çalışma, Python diliyle verimli veri modelleme, kurumsal kod standartları (PEP 8) ve dinamik içerik üretimi konularında profesyonel yetkinlik sergiler.",
    "42-Kocaeli-Django-2-Oob":
      "Bu proje, 42 Okulu Django Piscine eğitiminin üçüncü modülü olan nesne yönelimli programlama (OOP) ve dinamik içerik üretim sistemleri çalışmasıdır. Python'da sınıf hiyerarşisi tasarımı, kalıtım (inheritance), iç içe sınıflar (nested classes) ve özel hata yönetimi (exception handling) mimarilerini kapsar. Proje içeriğinde; regex tabanlı bir şablon motoru (template engine), Python nesneleriyle programatik HTML DOM oluşturma altyapısı ve hiyerarşik HTML yapılarını kurallara göre doğrulayan kapsamlı bir 'Page Validator' sistemi geliştirilmiştir. Bu çalışma, nesneye dayalı yazılım mimarisi tasarımı, yapısal veri doğrulama algoritmaları ve web tabanlı programatik içerik üretimi konularında profesyonel teknik yetkinlik sergiler.",

    // 42-Kocaeli-Unity-Piscine
    "42-Kocaeli-Unity-0-TheBasicsUnityTools":
      "Bu proje, 42 Okulu Unity Piscine eğitiminin ilk modülü olan temel oyun mekanikleri ve fizik simülasyonu çalışmasıdır. Unity oyun motoru üzerinde; Rigidbody tabanlı karakter kontrolü, Raycast ile zemin denetimi ve trigonometrik hesaplamalarla kurgulanmış orbital kamera sistemlerini kapsar. Proje içeriğinde; 'Floor is Lava' mekaniği temel alınarak, materyal bazlı texture animasyonları, trigger tabanlı çarpışma tespiti ve vektörel matematik hesaplamalarıyla dinamik bir 3D platform deneyimi geliştirilmiştir. Bu çalışma, Unity komponent mimarisi, fizik motoru entegrasyonu ve oyun döngüsü (game loop) yönetimi konularında profesyonel bir başlangıç yetkinliği sergiler.",
  },
  en: {
    // Yeditepe-University-Term-Projects
    UnoTermProject:
    ...
    // 42-Kocaeli-Mobile-Piscine
    "42-Kocaeli-Mobile-0":
      "This project is the introductory module of the 42 School Mobile Piscine training, focusing on Flutter and UI development. Using the Dart programming language and Flutter's widget system (Stateless/Stateful), it covers dynamic interface management, responsive design principles, and complex layout architectures. The project includes a fully functional calculator application enriched with a custom parsing algorithm based on operator precedence and comprehensive error management mechanisms. This work is a professional example that serves as an introduction to modern mobile application development processes and cross-platform UI design standards.",
    "42-Kocaeli-Mobile-1":
      "This project is the second module of the 42 School Mobile Piscine training, focusing on advanced user interface (UI) and navigation. It covers the development of a multi-page weather application using Flutter's 'TabBar' and 'TabController' structures, dynamic search systems, and advanced responsive design techniques based on 'MediaQuery' and 'LayoutBuilder'. The project demonstrates widget lifecycle management, integration of 'SingleTickerProviderStateMixin', and adaptive design strategies that adjust to screen sizes. This work is a comprehensive example designed to deepen understanding of complex mobile navigation flows and professional asset management topics.",
    "42-Kocaeli-Mobile-2":
      "This project is the third module of the 42 School Mobile Piscine training, focusing on API integration and advanced asynchronous data management. It covers fetching data from external weather APIs (Open-Meteo) with the 'http' library, managing real-time GPS location services with 'geolocator', and a modular asynchronous architecture based on 'FutureBuilder'. The application successfully demonstrates production-level mobile application standards such as autocomplete search suggestions, advanced error management mechanisms, and runtime permission control. This work is a comprehensive example that interacts with real-world data to produce efficient and scalable mobile solutions.",
    "42-Kocaeli-Mobile-3":
      "This project is the fourth module of the 42 School Mobile Piscine training, focusing on data visualization and advanced user interface design. Using the 'FL Chart' library, it presents a professional weather application featuring interactive charts, line charts showing temperature and wind speed trends, and customized background assets. The project successfully applies advanced topics such as visualization of complex data sets, performance optimization for graphics, and management of touch-interactive charts. This work is a high-quality mobile application example that transforms raw data into user-friendly and visually appealing models.",
    "42-Kocaeli-Mobile-4":
      "This project is the fifth module of the 42 School Mobile Piscine training, focusing on modern state management and backend integration. It covers reactive state management with the 'Riverpod' library, cloud-based PostgreSQL database operations with 'Supabase', and declarative navigation structures with 'Go Router'. The application successfully demonstrates advanced full-stack mobile application competencies such as OAuth 2.0 social login mechanisms via Google and GitHub, user-specific data filtering, and mood tracking. This work is a professional example of a route-guarded and scalable architecture mobile solution.",
    "42-Kocaeli-Mobile-5":
      "This project is the sixth module of the 42 School Mobile Piscine training, focusing on advanced user interface (UI) components and calendar integration. It covers interactive calendar management with the 'Table Calendar' library, a persistent bottom navigation bar with 'Shell Route' architecture via 'Go Router', and date-filterable content management features. The application successfully applies advanced mobile application patterns such as monthly, bi-weekly, and weekly view modes, multi-page architecture, and complex state coordination. This work offers a professional example of a component-based design and calendar-focused data organization in a mobile solution.",

    // 42-Kocaeli-Ocaml-Piscine
    "42-Kocaeli-OCAML-1-BasicSyntaxAndSemantics-0":
      "This project is the introductory module of the 42 School OCaml Piscine training, focusing on functional programming and basic language syntax. By utilizing OCaml's strong type system and immutability principles, it covers recursive functions, higher-order functions, and pattern matching techniques. The project content includes fast exponentiation algorithms, ROT-N encryption, and complex string manipulation operations successfully implemented with functional programming discipline. This work is a professional example that serves as a foundation for transitioning to the functional paradigm and solving algorithmic problems with recursive structures.",
    "42-Kocaeli-OCAML-2-RecursionAndNigherOrderFunctions-0":
      "This project is the second module of the 42 School OCaml Piscine training, focusing on advanced recursion and higher-order functions. It covers memory efficiency with 'tail recursion' optimization, mutual recursion, and flexible function structures with optional parameters. The project content includes complex mathematical algorithms such as Ackermann, Fibonacci, and Takeuchi, as well as numerical approximation methods with the Leibniz series successfully implemented with functional programming discipline. This work is a professional example that demonstrates deep technical proficiency in polymorphic function structures and algorithmic performance optimization.",
    "42-Kocaeli-OCAML-3-PatternMatchingAndDataTypes-0":
      "This project is the third module of the 42 School OCaml Piscine training, focusing on pattern matching and data type design. It covers advanced pattern matching techniques, including nested patterns, guards, and polymorphic variants. The project content includes complex data structures such as binary trees, linked lists, and custom option types successfully implemented with functional programming discipline. This work is a professional example that demonstrates how to model complex data and control flow with OCaml's powerful pattern matching capabilities.",
    "42-Kocaeli-OCAML-4-OcamlsModulsLanguage-1":
      "This project is the fourth module of the 42 School OCaml Piscine training, focusing on OCaml's module system and advanced language features. It covers modular programming with functors, abstract data types, and encapsulation principles. The project content includes a comprehensive implementation of a graph data structure with associated algorithms such as depth-first search (DFS) and breadth-first search (BFS), successfully utilizing OCaml's module system to create reusable and maintainable code. This work is a professional example that demonstrates how to structure complex applications with OCaml's powerful module capabilities.",
    "42-Kocaeli-OCAML-5-ImperativeFeatures-1":
      "This project is the fifth module of the 42 School OCaml Piscine training, focusing on OCaml's imperative features and mutable state management. It covers mutable data structures, reference cells, and side-effect management in a functional programming context. The project content includes a comprehensive implementation of a hash table with collision resolution strategies and a mutable graph structure with associated algorithms such as Dijkstra's shortest path, successfully utilizing OCaml's imperative features while maintaining functional programming principles. This work is a professional example that demonstrates how to effectively manage mutable state in OCaml while adhering to functional programming discipline.",
    "42-Kocaeli-OCAML-6-Functors-1":
      "This project is the sixth module of the 42 School OCaml Piscine training, focusing on functors and advanced module systems. It covers parametric module design, higher-order module signatures, and generic programming techniques. The project content includes implementations of 'Fixed-point' arithmetic systems, generic 'expression evaluator' for different data types, and customized implementations of standard library functors. This work demonstrates professional technical skills in code reusability, type abstraction, and zero-cost abstraction principles in large-scale software architectures.",
    "42-Kocaeli-OCAML-7-ObjectOrientedProgramming-1":
      "This project is the seventh module of the 42 School OCaml Piscine training, focusing on object-oriented programming (OOP) principles in OCaml. It covers class definitions, inheritance, and encapsulation using OCaml's object system. The project content includes a comprehensive implementation of a simple banking system with classes for accounts, transactions, and customer management, successfully utilizing OCaml's OOP features to create a modular and extensible application. This work is a professional example that demonstrates how to apply OOP principles in OCaml while maintaining functional programming discipline.",
    "42-Kocaeli-OCAML-8-ObjectOrientedProgramming-2":
      "This project is the eighth module of the 42 School OCaml Piscine training, focusing on advanced object-oriented programming (OOP) concepts in OCaml. It covers polymorphism, method overriding, and dynamic dispatch using OCaml's object system. The project content includes a comprehensive implementation of a simple game engine with classes for game objects, rendering, and input management, successfully utilizing OCaml's OOP features to create a flexible and extensible architecture. This work is a professional example that demonstrates how to effectively use OOP principles in OCaml while adhering to functional programming discipline.",
    "42-Kocaeli-OCAML-9-MonoidsAndMonads-3":
      "This project is the ninth module of the 42 School OCaml Piscine training, focusing on advanced functional programming concepts such as monoids and monads. It covers algebraic structures, type constructors, and functional composition techniques. The project content includes a comprehensive implementation of a 'Maybe' monad for handling optional values, a 'List' monad for managing collections, and a 'State' monad for encapsulating stateful computations, successfully utilizing OCaml's powerful type system and functional programming principles. This work is a professional example that demonstrates how to model complex computational patterns with monads in OCaml while maintaining code clarity and reusability.",

    // 42-Kocaeli-Django-Piscine
    "42-Kocaeli-Django-0-Initiation":
      "This project is the introductory module of the 42 School Django Piscine training, focusing on basic web technologies (HTML, CSS, JavaScript). It comprehensively covers the fundamentals of web development through semantic HTML5 structure, client-side form validation, CSS animations, and media management with jQuery (audio API). The project content includes Bash scripts that follow HTTP redirects, responsive blog designs, and JavaScript function chain management, developing both frontend and basic system interactions. This work is a professional example that serves as an introduction to modern web standards, DOM manipulation, and asset management topics.",
    "42-Kocaeli-Django-1-Starting":
      "This project is the second module of the 42 School Django Piscine training, focusing on Python fundamentals and data structures. It covers Python's basic data types, advanced dictionary manipulations, exception handling, file-based data processing, and command-line argument management. The project content includes sorting large data sets, reverse lookup techniques on complex data structures, and programmatically generating semantic HTML tables from periodic table data. This work is a professional example that demonstrates efficient data modeling with Python, corporate code standards (PEP 8), and dynamic content generation topics.",
    "42-Kocaeli-Django-2-Oob":
      "This project is the third module of the 42 School Django Piscine training, focusing on object-oriented programming (OOP) principles in Python. It covers class definitions, inheritance, encapsulation, and polymorphism with Python's OOP features. The project content includes a comprehensive implementation of a simple banking system with classes for accounts, transactions, and customer management, successfully utilizing Python's OOP capabilities to create a modular and extensible application. This work is a professional example that demonstrates how to apply OOP principles in Python while maintaining code clarity and reusability.",

    // 42-Kocaeli-Unity-Piscine
    "42-Kocaeli-Unity-0-TheBasicsUnityTools":
      "This project is the introductory module of the 42 School Unity Piscine training, focusing on basic game mechanics and physics simulation. It covers Rigidbody-based character control, Raycast for ground checking, and orbital camera systems designed with trigonometric calculations. The project content includes a dynamic 3D platform experience based on the 'Floor is Lava' mechanic, featuring material-based texture animations, trigger-based collision detection, and vector mathematics calculations. This work is a professional example that serves as an introduction to Unity's component architecture, physics engine integration, and game loop management principles.",
  },
};
